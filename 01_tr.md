...
Bir fonksiyona parametreleri parantez içine yazarak veririz. Parametrelerde asıl parametreyi değiştirip isteyip istemediğinize ve extra memory'nin sizin için sorun olup olmayacağına bağlı iki olay vardır: Call by value ve Call by reference. Burada stack ve heap ayrımı yapılır.
Stack kısa sürelidir LIFO'dur (Last In First Out) bu yüzden hızlıdır. Heap ise dynamic memory allocation'da kullanılır ve elle geri alan açmak gerekir (free).
...
Dereferans için illa asterisk kullanmak gerekmez.
...
typedef type (*name)(type parameter);
```
